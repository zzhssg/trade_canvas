...
Environment (optional):
  ...
  TRADE_CANVAS_ENABLE_STARTUP_KLINE_SYNC=0
  TRADE_CANVAS_STARTUP_KLINE_SYNC_TARGET_CANDLES=2000
  TRADE_CANVAS_ENABLE_MARKET_AUTO_TAIL_BACKFILL=1
  TRADE_CANVAS_ENABLE_STRICT_CLOSED_ONLY=1
  TRADE_CANVAS_ENABLE_READ_LEDGER_WARMUP=1
...
: "${TRADE_CANVAS_ENABLE_MARKET_AUTO_TAIL_BACKFILL:=1}"
: "${TRADE_CANVAS_ENABLE_STRICT_CLOSED_ONLY:=1}"
: "${TRADE_CANVAS_ENABLE_READ_LEDGER_WARMUP:=${TRADE_CANVAS_ENABLE_MARKET_AUTO_TAIL_BACKFILL}}"
: "${TRADE_CANVAS_MARKET_AUTO_TAIL_BACKFILL_MAX_CANDLES:=2000}"
: "${TRADE_CANVAS_MARKET_HISTORY_SOURCE:=freqtrade}"
: "${TRADE_CANVAS_ENABLE_CCXT_BACKFILL:=1}"
: "${TRADE_CANVAS_UVICORN_TIMEOUT_GRACEFUL_SHUTDOWN_S:=3}"
...
export TRADE_CANVAS_ENABLE_STARTUP_KLINE_SYNC
export TRADE_CANVAS_STARTUP_KLINE_SYNC_TARGET_CANDLES
export TRADE_CANVAS_ENABLE_MARKET_AUTO_TAIL_BACKFILL
export TRADE_CANVAS_ENABLE_STRICT_CLOSED_ONLY
export TRADE_CANVAS_ENABLE_READ_LEDGER_WARMUP
...
# Optionally reset local store artifacts to avoid stale debug data in repeated runs.
if [[ "${fresh_db}" == "1" ]]; then
...
if [[ "$reload" == "1" ]]; then
  # Limit reload scanning to backend code to avoid high CPU/IO when the repo has many files (frontend/node_modules, fixtures, large output artifacts, etc).
