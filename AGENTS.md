...
- 新增文件必须说明：它替代了哪个旧文件，或为什么现有文件不能扩展
...
- 禁止无说明的双向依赖；若因现实约束必须双向依赖，需补隔离层与后续拆解计划。
...
- 单个 TSX 组件不超过 400 行；超过 300 行必须在当前迭代先拆分职责，不得继续叠加新行为。
...
- 禁止创建少于 50 行的独立生产文件；确需例外时，必须在 plan 记录收益、替代方案与回滚方式。
...
- **交付三问（必须回答）**：
  - 如果删掉这个功能，需要改几个文件？（1-2 正常；3-5 必须解释；6+ 必须先给降耦合计划）
  - 新增的文件/类命名是否一眼可懂？（若否，交付前必须重命名或补充语义）
  - 数据流是否单向？（若存在双向依赖，必须说明隔离策略与拆解时间点）

### 禁止事项（硬刹车）

- 未写清“验收/证据/回滚”就大改结构或引入新链路。
- 为了赶进度破坏契约边界（“先实现再说，之后再清理”默认会变成永久技术债）。
- 无收益说明地引入超过 2 层抽象嵌套或双向依赖。
- 没跑过上面的门禁就宣称完成/交付。
