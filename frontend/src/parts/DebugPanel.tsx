...
type SeriesHealthGap = { prev_time: number; next_time: number; delta_seconds: number; missing_candles: number };
type SeriesHealthBucket = { bucket_open_time: number; expected_minutes: number; actual_minutes: number; missing_minutes: number };
type SeriesHealth = {
  series_id: string;
...
const HEALTH_QUERY = "max_recent_gaps=3&recent_base_buckets=4";
const BTN_CLASS =
  "rounded-md border border-white/10 bg-black/20 px-2 py-1 text-[11px] text-white/80 hover:bg-white/10 focus:outline-none focus-visible:ring-2 focus-visible:ring-sky-500/60";
...
function isDebugEvent(value: unknown): value is DebugEvent {
  ...
  if (typeof value.event !== "string" || !value.event || typeof value.message !== "string") return false;
...
function parseWsPayload(payload: unknown): DebugWsSnapshot | DebugWsEvent | null {
  ...
  if (payload.type === "debug_snapshot") {
    if (!Array.isArray(payload.events)) return null;
    const events: DebugEvent[] = [];
    for (const event of payload.events) {
      if (!isDebugEvent(event)) return null;
      events.push(event);
    }
    return { type: "debug_snapshot", events };
  }
  if (payload.type === "debug_event" && isDebugEvent(payload.event)) return { type: "debug_event", event: payload.event };
  return null;
}
...
const healthPath = (seriesId: string) => `/api/market/debug/series_health?series_id=${encodeURIComponent(seriesId)}&${HEALTH_QUERY}`;
...
export function DebugPanel() {
  ...
  const refreshHealth = useCallback(async () => {
    ...
    setHealthError(null);
    try {
      setHealth(await apiJson<SeriesHealth>(healthPath(seriesId)));
    } catch (e: unknown) {
      setHealthError(e instanceof Error ? e.message : "load failed");
      setHealth(null);
    } finally {
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      setHealthError(null);
      try {
        const payload = await apiJson<SeriesHealth>(healthPath(seriesId));
  ...
  useEffect(() => {
    ...
    ws.onclose = ws.onerror = () => alive && setWsState("disconnected");
    ws.onmessage = (evt) => {
      if (!alive || typeof evt.data !== "string") return;
      let parsed: DebugWsSnapshot | DebugWsEvent | null = null;
      try {
        parsed = parseWsPayload(JSON.parse(evt.data) as unknown);
      } catch {
        return;
      }
      if (!parsed) return;
      if (parsed.type === "debug_snapshot") {
        for (const e of parsed.events) logDebugEvent({ ...e, source: "backend" });
      } else {
  ...
  const filtered = useMemo(() => {
    ...
    return events.filter((e) => {
      ...
      if (!q) return true;
      return `${e.event} ${e.message} ${e.series_id ?? ""}`.toLowerCase().includes(q);
    });
  }, [events, filter, query]);
  ...
  const copyText = useMemo(
    () =>
      filtered
        .map((e) => `${formatTs(e.ts_ms)} [${e.source}] [${e.pipe}] ${e.event}${e.series_id ? ` ${e.series_id}` : ""} ${e.message}`)
        .join("\n"),
    [filtered]
  );

  if (!ENABLE_DEBUG_TOOL) return <div className="text-xs text-white/60">Debug tool disabled (VITE_ENABLE_DEBUG_TOOL != 1)</div>;

  return (
    <div className="flex flex-col gap-2" data-testid="debug-panel">
      <div className="flex flex-wrap items-center gap-2">
        <div className="rounded-md border border-white/10 bg-black/20 px-2 py-1 font-mono text-[11px] text-white/80">ws:{wsState}</div>
        <div className="rounded-md border border-white/10 bg-black/20 px-2 py-1 font-mono text-[11px] text-white/60">series:{seriesId}</div>
        <button type="button" className={`ml-auto ${BTN_CLASS}`} onClick={clear}>Clear</button>
        <button type="button" className={BTN_CLASS} onClick={() => void refreshHealth()} title="Refresh series health">Health</button>
        <button
          type="button"
          className={BTN_CLASS}
      ...
      <div className="flex items-center gap-2">
        ...
        <label className="ml-auto flex items-center gap-2 text-[11px] text-white/70">
          <input type="checkbox" checked={autoScroll} onChange={toggleAutoScroll} />Auto-scroll
        </label>
      </div>
      ...
      <div className="rounded-xl border border-white/10 bg-black/10 p-2 text-[11px] text-white/75">
        {healthLoading ? (
          ...
          <div className="flex flex-col gap-1">
            <div className="font-mono text-white/85">head:{health.head_time ?? "null"} lag_s:{health.lag_seconds ?? "null"} count:{health.candle_count}</div>
            <div className="font-mono text-white/70">gaps:{health.gap_count} max_gap_s:{health.max_gap_seconds ?? "null"} base:{health.base_series_id}</div>
            {health.recent_gaps.length > 0 ? (
              <div className="font-mono text-white/65">recent gaps:{health.recent_gaps.map((g) => <span key={`${g.prev_time}:${g.next_time}`} className="ml-2">{`[${g.prev_time}->${g.next_time} miss=${g.missing_candles}]`}</span>)}</div>
            ) : null}
            {health.base_bucket_completeness.length > 0 ? (
              <div className="font-mono text-white/65">base buckets:{health.base_bucket_completeness.map((b) => <span key={b.bucket_open_time} className="ml-2">{`[${b.bucket_open_time} ${b.actual_minutes}/${b.expected_minutes}]`}</span>)}</div>
            ) : null}
          </div>
        ) : (
            ...
            {filtered.map((e, idx) => (
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <div className="w-[62px] shrink-0 rounded-md border border-white/10 bg-black/20 px-1.5 py-0.5 text-center font-mono text-white/70">{e.source}</div>
                  <div className={[
                    "w-[52px] shrink-0 rounded-md border px-1.5 py-0.5 text-center font-mono",
                    e.pipe === "read" ? "border-sky-500/25 bg-sky-500/10 text-sky-200" : "border-amber-500/25 bg-amber-500/10 text-amber-200"
                  ].join(" ")}>
                    {e.pipe}
                  </div>
                ...
                <div className="mt-1 flex flex-col gap-1">
                  ...
                  {e.data ? (
                    <details className="rounded-md border border-white/10 bg-black/20 p-2 text-white/70">
                      ...
                      <pre className="mt-2 whitespace-pre-wrap break-words font-mono text-[10px] leading-snug">{JSON.stringify(e.data, null, 2)}</pre>
                    </details>
                  ) : null}
                </div>
              </div>
