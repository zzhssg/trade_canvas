...
from backend.app.worktree_manager import WorktreeManager
...
def test_list_worktrees_marks_canonical_main_from_git_common_dir(
    ...
    monkeypatch.setattr("backend.app.worktree_manager.subprocess.run", fake_run)
    monkeypatch.setattr(manager, "_read_index", lambda: {"version": 1, "allocations": {}, "active_services": {}})
    monkeypatch.setattr(manager, "_read_metadata", lambda _wid: None)
...
def test_start_services_passes_frontend_port_to_backend_start(
    ...
    monkeypatch.setattr(manager, "_start_backend", fake_start_backend)
