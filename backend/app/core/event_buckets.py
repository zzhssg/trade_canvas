...
class EventBucketSpecLike(Protocol):
    factor_name: str
    event_kind: str
    bucket_name: str
    sort_keys: tuple[str, str] | None


def build_event_bucket_config(
    *,
    bucket_specs: Iterable[EventBucketSpecLike],
    conflict_prefix: str,
    ...
    for spec in bucket_specs:
        ...
        if existing_bucket is not None and existing_bucket != bucket_name:
            raise RuntimeError(f"{conflict_prefix}_bucket_conflict:{factor_name}:{event_kind}")
        by_kind[key] = bucket_name
        bucket_names.add(bucket_name)

        if spec.sort_keys is None:
            continue
        ...
        if existing_sort is not None and existing_sort != sort_pair:
            raise RuntimeError(f"{conflict_prefix}_bucket_sort_conflict:{bucket_name}")
        sort_keys[bucket_name] = sort_pair

    return by_kind, sort_keys, tuple(sorted(bucket_names))


def collect_event_buckets(
    *,
    rows: Iterable[Any],
