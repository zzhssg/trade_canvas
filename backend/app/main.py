...
def create_app() -> FastAPI:
    ...
    def _require_overlay_pkg_enabled() -> None:
        if not app.state.overlay_pkg_service.enabled():
            raise HTTPException(status_code=404, detail="not_found")

    @app.get("/api/replay/overlay_package/read_only", response_model=ReplayOverlayPackageReadOnlyResponseV1)
    def replay_overlay_package_read_only(
        req: ReplayOverlayPackageReadOnlyRequestV1 = Depends(),
    ) -> ReplayOverlayPackageReadOnlyResponseV1:
        _require_overlay_pkg_enabled()
        status, job_id, cache_key, meta, hint = app.state.overlay_pkg_service.read_only(
            series_id=req.series_id,
            to_time=req.to_time,
    ...
    @app.post("/api/replay/overlay_package/build", response_model=ReplayOverlayPackageBuildResponseV1)
    def replay_overlay_package_build(req: ReplayOverlayPackageBuildRequestV1) -> ReplayOverlayPackageBuildResponseV1:
        _require_overlay_pkg_enabled()
        status, job_id, cache_key = app.state.overlay_pkg_service.build(
            series_id=req.series_id,
            to_time=req.to_time,
            window_candles=req.window_candles,
            window_size=req.window_size,
    ...
    @app.get("/api/replay/overlay_package/status", response_model=ReplayOverlayPackageStatusResponseV1)
    def replay_overlay_package_status(
        job_id: str = Query(..., min_length=1),
        include_delta_package: int = Query(0, ge=0, le=1),
    ) -> ReplayOverlayPackageStatusResponseV1:
        _require_overlay_pkg_enabled()
        payload = app.state.overlay_pkg_service.status(
            job_id=job_id,
            include_delta_package=bool(int(include_delta_package)),
        )
        return ReplayOverlayPackageStatusResponseV1.model_validate(payload)

    @app.get("/api/replay/overlay_package/window", response_model=ReplayOverlayPackageWindowResponseV1)
