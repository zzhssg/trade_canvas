...
from .overlay_store import OverlayInstructionVersionRow, OverlayStore
...
def _is_relevant_for_range(item: OverlayInstructionPatchItemV1, *, cutoff_time: int, to_time: int) -> bool:
    """
    Filter catalog items to reduce payload size.

    The replay range is fixed (<= 2000 closed candles), so we only keep instructions that can intersect
    [cutoff_time, to_time] under the MVP active rules.
    """
    ...
    # Unknown kinds: keep conservatively (future-proof).
    return True
...
def compute_active_ids_v1(
    ...
    *,
    ...
    """
    MVP active rule (monotonic-add within a fixed replay range):

    - marker: active when cutoff_time <= marker_time <= to_time (marker_time comes from definition.time)
    - polyline: active when
        - max_point_time >= cutoff_time (intersects range)
        - min_point_time <= to_time (becomes active once earliest point is in the past)
    """
    out: list[str] = []
