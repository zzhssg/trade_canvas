...
type AnchorSwitchV1 = {
  ...
  reason: string              // "strong_pen" | "zhongshu_entry" | "manual" | ...
...
- **稳定性门禁**：默认只记录稳定切换；但 `strong_pen` 允许 `kind=="candidate"`（用于捕捉强度领先的候选笔），仍需满足 append-only 与可见性门禁。
...
- `strong_pen`：在可见的 pens（confirmed + candidate）中选择“力度最大”的一根作为初始锚（力度口径必须唯一：例如 `abs(end_price-start_price)`；更复杂口径后续版本化）。
