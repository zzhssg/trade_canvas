---
title: 第22关：从依赖注入到容器边界，讲透如何避免全局状态失控
status: done
created: 2026-02-11
updated: 2026-02-11
---

# 第22关：从依赖注入到容器边界，讲透如何避免全局状态失控

上一关你学了"控制面怎么装配"。这一关解决"依赖怎么管、边界怎么守"。
...
这就像一个电影剧组没有道具组——演员自己上街买道具、自己改剧本、自己搭布景。拍出来的东西能看，但换个演员就全乱了。

trade_canvas 的解法是：**把"拿依赖"这件事集中到容器边界，只允许一条受控路径。**
...
想象一个电影剧组。开拍前，道具组把所有道具统一准备好、编号入库；拍摄时，演员只管从道具间领取，不许自己上街买。这就是"组合根"。
...
容器本身就像一份"活的架构目录"——就像剧组的道具清单，一眼就知道这部戏需要什么。
...
这相当于把大系统拆成"主容器 + 子容器"——就像剧组里有总道具组，也有各场景的分道具组，各管各的清单，最后统一汇总。
