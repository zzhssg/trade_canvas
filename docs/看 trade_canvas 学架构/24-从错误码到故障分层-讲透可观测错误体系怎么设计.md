---
title: 第24关：从错误码到故障分层，讲透可观测错误体系怎么设计
status: done
created: 2026-02-11
updated: 2026-02-11
---

# 第24关：从错误码到故障分层，讲透可观测错误体系怎么设计

上一关你学了"可回滚后端治理的四层保险"。这一关深入其中一层：错误体系。

先讲一个场景。凌晨两点，你被告警叫醒，打开监控一看：前端全是 500，后端日志一堆 stacktrace。你盯着屏幕问自己：这到底是用户传了个错参数，还是数据库挂了，还是某个开关没开？

如果你的错误体系只有一种表达——"失败了"——那你只能一行行翻日志猜。

这就像快递分拣中心如果不分拣：地址写错的、超重的、易碎的、正常的全堆在一条传送带上，分拣员看到的只有"包裹来了"，没有任何分类标签。结果是每个包裹都要拆开检查，效率极低，紧急件可能被压在最底下。

trade_canvas 这套错误体系的核心价值，就是把"异常"变成"可分层分拣信号"——让你 5 分钟内定位层级和责任边界。
...
在本项目里，至少有四类"非成功状态"，就像快递分拣中心的四条通道：

1. **输入层错误**：请求格式或参数不合法（地址写错了——退回寄件人）。
2. **状态层错误**：系统状态暂时不满足不变量，最典型是 `ledger_out_of_sync:*`（包裹在中转站还没到——稍后再查物流）。
3. **能力层错误**：功能没开、容量满、资源不可用（该线路停运了——换一条线路）。
4. **内部层错误**：代码/依赖执行失败，`ingest_pipeline_failed:*` 等（分拣机坏了——需要维修工）。

这套分层最关键的收益是：

**调用方可以根据层级决定动作，而不是看到 error 就盲重试。**

地址错了你重试一万次也没用；包裹在路上你等一会儿就好；线路停运你换一条；分拣机坏了你叫维修。每种错误的正确应对完全不同。

---

## 2. HTTP 通道：ServiceError 做内部分型，Route 统一映射

核心机制在 `backend/app/service_errors.py`。
...
这是一个明确权衡：客户端负担小，但内部仍保留强分型。就像快递面单上只写"请送到 XX 小区"，不会把内部分拣编码印给收件人看。
...
**HTTP 偏"状态码 + detail"，WS 偏"协议内结构化错误码"。** 两条通道各有分工，就像普通件走陆运面单、特快件走空运面单——格式不同，但都能追踪。
...
这就是优秀错误体系的标志——不把"可恢复状态问题"误标成"服务器内部错误"。就像快递查询显示"中转中"不是"丢件"——包裹还在路上，只是没到终点。
...
- 关掉就"彻底不可见"，不是半可用（仓库关门就锁门，不是留半扇门让人进来干等）；
- 满载就"明确拒绝"，不是拖死系统（传送带超载就报警停机，不是硬塞然后卡住整条线）。
...
就像快递状态查询：显示"待揽收"不是"配送失败"，而是"还没开始"这个正常状态。
...
这让你看到的不再是"失败了"，而是"失败在哪一步，补偿做到哪一步"。就像快递运单不只写"配送异常"，而是记录每一个中转站的扫描时间、在哪个节点出了问题、是否已退回。
