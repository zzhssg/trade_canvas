...
涉及链路（会互相影响）：
- 前端 `ChartView` 的数据源切换（历史：`/api/plot/delta` → `/api/overlay/delta`；现状：统一为 `/api/draw/delta`，且旧接口已于 2026-02-05 删除）
...
- 例如：从 `plot_delta` 迁移到 `overlay_delta`，应同步：
  - 前端 API client 使用点
  - E2E 断言与等待点（现状应等待 `/api/draw/delta` 或 UI 暴露的 overlay/draw 计数）
...
开发前：
- [ ] 明确本次 E2E 的“唯一入口与唯一数据源”（例如 `draw_delta`），并写到 plan 的 E2E 用户故事里。
...
- [ ] 给 E2E 增加“关键 endpoint 命中”的日志/断言（例如必须命中 `/api/draw/delta`）。
