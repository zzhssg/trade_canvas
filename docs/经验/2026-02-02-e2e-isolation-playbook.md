...
- 快速命令（示例）：
  - `rg -n "GET /api/overlay/delta|GET /api/plot/delta|GET /api/market/candles" output/e2e_acceptance_*.log`
...
- DB：每个 spec 或每个 worker 使用独立 sqlite（最稳），至少每次 run 清空 DB
  - 当前脚本只会清空默认路径 `backend/data/market_e2e.db`；如果换了端口/复用服务器，要确认 DB 路径是否仍一致

3) 用“可见的 UI data-attributes”做断言，但必须与契约同步
- 如果 Chart 数据源从 `plot_delta` 切到 `overlay_delta`：
  - E2E 必须断言 `/api/overlay/delta` 命中（或 UI 提供 overlay 计数）
  - 旧的 `plot_delta` 断言要么移除，要么作为兼容分支（否则必然卡死）
